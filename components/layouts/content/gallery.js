  ...
  return (
    <>
      ...
      {data.content.length > 1 ? (
        <Grid>
          <Grid.Col span={4}>
            <Button disabled={currIndex < 1} onClick={() => onPaginationClick(false)} fullWidth size="xl" variant="gradient" gradient={{ from: "#f45b42", to: "cyan" }}>
              Prev
            </Button>
          </Grid.Col>
          <Grid.Col span={8}>
            <Button disabled={currIndex >= data.items_count - 1} onClick={() => onPaginationClick(true)} fullWidth size="xl" variant="gradient" gradient={{ from: "cyan", to: "indigo" }}>
              Next
            </Button>
          </Grid.Col>
        </Grid>
